...
public class FaaSClientWithDpopAuthTest {
    ...
    private String authHeader = "some_access_token";
    ...
    public void before() throws Exception {
        ...
        when(authDPoPSignatureBuilder.getAccessTokenInternal(anyString())).thenReturn(authHeader);
    ...
    public void invokeViaUUIDWithRequestId() throws Exception {
        ...
        assertTrue("Lambda invocation with wrong authorization header", httpHeaderCaptor.getValue().get(
                "Authorization").equals("DPoP " + authHeader));
        assertTrue("Lambda invocation with wrong DPoP header", httpHeaderCaptor.getValue().get(
                        "DPoP").equals(dpopHeader));
    ...
    public void isImplementedThrowFaaSException() throws FaaSException, TokenGenerationException {
        try {
            when(authDPoPSignatureBuilder.getAccessTokenInternal(accountId)).thenThrow(new TokenGenerationException("could not generate " +
                    "token"));
            client.isImplemented(externalSystem, event, optionalParams);
        } catch (Exception ex) {
            verify(metricCollectorMock, times(1)).onIsImplementedFailure(eq(externalSystem), anyFloat(),
